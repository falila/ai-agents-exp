...
def procurement_agent_node(state: AgentCommerceState) -> Dict[str, Any]:
    ...
    try:
        ...
        total_price = target_item["unit_price_xrp"] * state.get("quantity", 1)
        invoice_res = generate_invoice(prod_id, total_price)
        ...
        # Fallback path: if the LLM is unavailable/slow, proceed deterministically
        # when the requested product ID is present in inventory.
        target_item = next((item for item in inventory_list if item["product_id"] == prod_id), None)
        if target_item is not None:
            ...
            return {
                ...
                "invoice_id": invoice_res["invoice_id"],
                "price_drops": int(invoice_res["price"] * 1_000_000),
                "procurement_fallback_reason": f"LLM unavailable: {e}",
            }
